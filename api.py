...
def get_training_data():
    url = "http://localhost:3000/api/v1/listings/training_data"
    r = urllib2.urlopen(url)
    datas = json.loads(r.read())
    training_inputs = []
    training_results = []
    for data in datas:
        training_inputs.append([data['offer_price'], data['stock_market'], data['business_category'], data['population']])
...
def get_test_data():
    url = "http://localhost:3000/api/v1/listings/test_data"
    r = urllib2.urlopen(url)
    datas = json.loads(r.read())
    training_inputs = []
    training_results = []
